...
class Scanner
{
    ...
    public function collect(AnnotationReader $reader, ReflectionClass $reflection)
    {
        ...
        foreach ($properties as $property) {
            $filter[$property->getName()] = $property->getName();
        ...
        // Parse properties annotations
        try {
            $properties = ReflectionManager::reflectClass($className)->getProperties();
            count($filter) == count($properties) && $properties = [];
            foreach ($properties as $property) {
                if ($filter[$property->getName()] ?? false) {
                    continue;
                }
